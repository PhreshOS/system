...
test.each(["wallpaper", "start-menu"] as const)("%s exposes only concepts understood by its presentation", async layer => {
    ...
    expect(presentations.read("own", "title")).toBe("Original")
    expect(presentations.read("own", "layer")).toBe(layer)
    for (const property of ["position", "size", "minimized", "maximized", "header", "frame", "front"] as const) {
    ...
    const unsupported = [
        ...
        () => presentations.geometry("own", { position: { x: 1, y: 2 }, size: { width: 1, height: 2 } }),
        () => presentations.minimize("own", true),
        () => presentations.maximize("own", false),
        () => presentations.header("own", false),
        () => presentations.frame("own", true),
    ...
    expect(presentations.projection("own").title).toBe("Changed")
    expect(presentations.projection("own").position).toEqual({ x: 0, y: 0 })
})
...
test.each(["under", "over"] as const)("%s presents geometry and frame without a header", async layer => {
    ...
    expect(() => presentations.header("own", false)).toThrow(new RegExp(`${layer} layer cannot apply header`))

    await presentations.frame("own", { color: "primary", radius: "full" })
...
test("a standard presentation reports its permanent frame without allowing frame changes", async () => {
    ...
    expect(() => presentations.frame("own", false)).toThrow(/window layer cannot apply frame/)
})
...
test("unsupported presentation events remain silent", () => {
    ...
    expect(received).toEqual(["changeTitle"])
})
