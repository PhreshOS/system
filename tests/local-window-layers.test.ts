...
function fixture(layer: WindowLayer) {
    const client = (layer: WindowLayer) => ({ window: {
        layer, title: "Original", header: true, position: { x: 20, y: 30 }, size: { width: 320, height: 240 },
...
test.each(["wallpaper", "start-menu"] as const)("%s local geometry and visibility belong to Desktop, not authoritative state", async layer => {
    ...
    expect(initial).toMatchObject({ title: "", header: true, position: { x: 0, y: 0 }, size: { width: "100%", height: "100%" }, minimized: false, maximized: true, depth: 0, surface: null })
    const operations = [
        ...
        () => windows.title("own", "Local"), () => windows.header("own", false), () => windows.raise("own"),
...
test.each(["under", "over"] as const)("%s follows allowed properties without copying standard header state", async layer => {
    const { own, target, entries, windows } = fixture(layer)
    expect(() => windows.title("own", "Local")).toThrow(/local title/)
    expect(() => windows.header("own", false)).toThrow(/local header/)
    await windows.follow("own", "target")
    target.window.title = "Changed"
    target.window.header = false
    target.window.position = { x: 100, y: 200 }
    windows.reconcile(entries)
    expect(windows.projection("own").title).toBe("")
    expect(windows.projection("own").header).toBe(true)
    ...
    windows.header("target", false)
    expect(windows.projection("target").title).toBe("Local title")
    expect(windows.projection("target").header).toBe(false)
})
