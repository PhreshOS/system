...
export default function OverflowRow({ children, backwardLabel = "Scroll backward", forwardLabel = "Scroll forward", className, ...props }: OverflowRowProps) {
    ...
    function scroll(direction: -1 | 1) {
        ...
        view.scrollBy({ left: direction * view.clientWidth * 0.8, behavior: reducedMotion ? "auto" : "smooth" })
    }
...
function ScrollButton({ label, controls, direction, disabled, reducedMotion, onClick }: ScrollButtonProps) {
    ...
    return <button
        ...
        className={`grid size-7 shrink-0 place-items-center rounded-lg border border-white/45 bg-white/55 text-slate-700 shadow-sm outline-none backdrop-blur-sm focus-visible:ring-2 focus-visible:ring-white disabled:opacity-35 ${reducedMotion ? "" : "transition-colors hover:bg-white/80 active:scale-95"}`}

    >

        <svg aria-hidden="true" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" className="size-3">
