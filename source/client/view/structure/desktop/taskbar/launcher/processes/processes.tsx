...
export default function Processes() {
    ...
    return <div role="group" aria-label="Processes" className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)] p-2">
        ...
        <h3 className="px-3 py-2 text-xs font-medium opacity-60">Processes · {processes.length}</h3>

        {processes.length ? <ul className="m-0 min-h-0 list-none overflow-y-auto p-0">

            {processes.map(process => <ProcessItem key={process.identity} process={process} program={programsByIdentity.get(process.program)} />)}

        </ul> : <p className="m-0 grid min-h-32 place-items-center px-3 py-8 text-center text-sm opacity-50">No Processes</p>}
